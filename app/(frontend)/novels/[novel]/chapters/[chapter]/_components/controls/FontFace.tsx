...
const availableFonts: Record<ReadingUtilsContextValue['fontFace'], string> = {
    lora: 'Lora',
...
const FontFace: FC<Props> = ({ className }) => {
    const { fontFace, setFontFace } = useReader();
    ...
    return (
        ...
        >
            <h3 className="text-lg font-bold">Font Face</h3>

            <Listbox value={fontFace} onChange={setFontFace}>
                <div className="relative w-32 mt-1">
                    <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-pointer sm:text-sm">
                        <span className="block truncate">{availableFonts[fontFace]}</span>
                    ...
                    >
                        <Listbox.Options className="absolute z-20 w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 focus:outline-none sm:text-sm">
                            {Object.entries(availableFonts).map(([font, name]) => (
                                <Listbox.Option
                                    key={font}
