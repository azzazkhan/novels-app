...
const availablePlaybackSpeeds: Array<ReadingUtilsContextValue['playbackSpeed']> = [
    1, 1.5, 2, 2.5, 3,
];

const PlaybackSpeed: FC<Props> = ({ className }) => {
    const { playbackSpeed, setPlaybackSpeed } = useReader();
    ...
    return (
        ...
        >
            <h3 className="text-lg font-bold">Playback Speed</h3>
            ...
            <Listbox value={playbackSpeed} onChange={setPlaybackSpeed}>
                <div className="relative w-32 mt-1">
                    <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-pointer sm:text-sm">
                        <span className="block truncate">{playbackSpeed}x</span>
                    ...
                    >
                        <Listbox.Options className="absolute z-20 w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 focus:outline-none sm:text-sm">
                            {availablePlaybackSpeeds.map((size) => (
                                <Listbox.Option
                                    key={size}
