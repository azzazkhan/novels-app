...
const availableSpacings: Array<[string, ReadingUtilsContextValue['lineHeight']]> = [
    ['none', 1],
    ['tight', 1.25],
    ['snug', 1.375],
    ['normal', 1.5],
...
const LineHeight: FC<Props> = ({ className }) => {
    const { lineHeight, setLineHeight } = useReader();
    ...
    return (
        ...
        >
            <h3 className="text-lg font-bold">Line Height</h3>

            <Listbox value={lineHeight} onChange={setLineHeight}>
                <div className="relative w-32 mt-1">
                    <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-pointer sm:text-sm">
                        <span className="block truncate capitalize">
                            {availableSpacings.find((height) => lineHeight === height[1])![0]}
                        </span>
                    ...
                    >
                        <Listbox.Options className="absolute z-20 w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 focus:outline-none sm:text-sm">
                            {availableSpacings.map(([name, height]) => (
                                <Listbox.Option
                                    key={height}
