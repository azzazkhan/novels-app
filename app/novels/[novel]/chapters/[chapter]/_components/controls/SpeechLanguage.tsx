...
const availableTranslations: Record<ReadingUtilsContextValue['speechLanguage'], string> = {
    dutch: 'Dutch',
    english: 'English',
    espanol: 'Espanol',
    french: 'French',
...
const SpeechLanguage: FC<Props> = ({ className }) => {
    const { speechLanguage, setSpeechLanguage } = useReader();
    ...
    return (
        ...
        >
            <h3 className="text-lg font-bold">Voice Language</h3>

            <Listbox value={speechLanguage} onChange={setSpeechLanguage}>
                <div className="relative w-32 mt-1">
                    <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-white rounded-lg shadow-md cursor-pointer sm:text-sm">
                        <span className="block truncate">
                            {availableTranslations[speechLanguage]}
                        </span>
                    ...
                    >
                        <Listbox.Options className="absolute z-20 w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 focus:outline-none sm:text-sm">
                            {Object.entries(availableTranslations).map(([font, name]) => (
                                <Listbox.Option
                                    key={font}
